...
class dbconnection
{
  ...
  public function doQuery($sql, $attributes=NULL)
	{
		$prepared = $this->link->prepare($sql);
	...
	public function doQueryAll($sql, $attributes=NULL) 
	{
		$prepared = $this->link->prepare($sql);
		$prepared->execute($attributes);
		$res = $prepared->fetchAll( PDO::FETCH_ASSOC );
	...
	public function doQueryObject( $sql, $className, $attributes=NULL )
	{
		$prepared = $this->link->prepare( $sql );
		$prepared->execute($attributes);
	...
	public function doQueryAllObject( $sql, $className, $attributes=NULL )
	{
		$prepared = $this->link->prepare( $sql );
		$prepared->execute($attributes);
		$res = $prepared->fetchAll( PDO::FETCH_CLASS, $className );
